...
function Kill-Port {
    ...
    if ($process) {
        Write-Host "🔧 Matar proceso en puerto $port (PID: $process)" -ForegroundColor Yellow
...
try {
    $response = Invoke-WebRequest -Uri "http://localhost:3000/api/devices" -Method GET -TimeoutSec 5 -ErrorAction Stop
    Write-Host "✅ Backend iniciado correctamente" -ForegroundColor Green
} catch {
    Write-Host "❌ Error: Backend no inició correctamente" -ForegroundColor Red
...
$frontendProcess = Start-Process -FilePath "npm" -ArgumentList "run", "dev" -WorkingDirectory "$projectRoot\frontend" -PassThru -WindowStyle Minimized
Start-Sleep -Seconds 3
...
Write-Host "" -ForegroundColor White
Write-Host "O presiona Ctrl+C para detener este script (los servicios seguirán corriendo)" -ForegroundColor Gray
...
try {
    while ($true) {
        ...
        if ($frontendProcess.HasExited) {
            Write-Host "⚠️ Frontend se detuvo inesperadamente" -ForegroundColor Red
        }
    }
} finally {
    # Limpieza al salir
