    ...
    <div className="flex w-[304px] flex-col rounded-2xl border border-cyan-500/20 bg-slate-900/60 p-4 shadow-[0_0_25px_rgba(6,182,212,0.12)] backdrop-blur-md">
      ...
      <div className="flex items-start justify-between gap-2 border-b border-cyan-500/10 pb-3">
        <div className="min-w-0">
          <div className="text-[9px] font-bold uppercase tracking-[0.22em] text-slate-500">Dispositivo</div>
          <div className="truncate font-mono text-[15px] font-bold text-slate-100" title={entity?.id}>
            {entity?.id || 'SIN SELECCIÓN'}
          </div>
          <div className="mt-0.5 text-[10px] text-slate-500">{entity?.model || '—'}</div>
        </div>
        <span
          className="flex shrink-0 items-center gap-1.5 rounded-full border px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.14em]"
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <TelemetryCell
          ...
          label="Velocidad"
          value={<span className="font-mono text-[18px] font-extrabold text-[#f8fafc]">{Math.round(entity?.speed ?? 0)}<span className="ml-1 text-[9px] font-bold text-slate-500">km/h</span></span>}
          accent={online ? '#06b6d4' : '#64748b'}
        />
        <TelemetryCell
          icon={<Activity size={13} />}
          label="Precisión"
          value={<span className="font-mono text-[18px] font-extrabold text-[#f8fafc]">{accuracy ?? '—'}<span className="ml-1 text-[9px] font-bold text-slate-500">m</span></span>}
          accent="#10b981"
        />
      </div>
      ...
      <div className="mt-2 rounded-lg border border-cyan-500/10 bg-[#0a1220]/60 p-2.5">
        <div className="mb-1.5 flex items-center justify-between">
          <span className="flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-[0.2em] text-slate-500">
      ...
      <div className="mt-auto flex items-center gap-2 border-t border-cyan-500/10 pt-2.5">
        ...
        {onEditVehicle && (
          <button
            ...
            className="shrink-0 rounded-md border border-cyan-500/25 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-cyan-400 transition-all hover:bg-cyan-500/10"
        ...
        {onDeleteEntity && (
          <button
            ...
            className="shrink-0 rounded-md border border-red-500/25 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-red-400 transition-all hover:bg-red-500/10"
          >
            Eliminar
          </button>
        )}
